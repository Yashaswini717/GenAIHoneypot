...
class NodeRenderer:
    ...
    def render_identity(self) -> None:
        ...
        # Peer addresses are deliberately NOT baked in.
        #
        # Each attacker now gets their own network with its own subnet, so an
        # address fixed at build time is not on the network they are actually
        # on. Baking them produced two conflicting entries per host in
        # /etc/hosts -- the broker's real one and a stale 10.60.0.x -- which
        # is both broken and conspicuous. The broker injects the live
        # addresses via extra_hosts at container creation instead.
    ...
    def render_decoys(self) -> None:
        ...
        """
        chain = self.identity.get("decoy_chain")
        if not chain:
            # Only the entry node carries the pivot chain. The nodes an
            # attacker reaches *by* pivoting do not need to hand out another
            # key -- they are the reward, and their own bait lives in their
            # role content.
        ...
        #
        # A REAL key, generated by tools/bootstrap.py. A synthetic blob looks
        # right under `cat` and then fails to parse the moment `ssh -i` is
        # pointed at it -- dying at exactly the point the chain exists to
        # reach. Its public half authorises the `deploy` account on the proxy
        # and nothing else.
        planted = Path("/tmp/build/decoy_deploy_key")
        material = (
            planted.read_text(encoding="utf-8")
            if planted.exists()
            else _openssh_private_key(self.rng)
        )
        self.write(
            key_path,
            material,
    ...
    def render_environment(self) -> None:
        ...
        """
        # Role decides which parts of the estate this node actually holds.
        # A database server has no /opt/deploy, and the jump host has no
        # web root -- putting everything everywhere is how an estate stops
        # looking like an estate.
        role = self.identity["host"].get("kind", "jump")
        if role != "jump":
            return
    ...
    # -- role-specific estate ----------------------------------------------

    def render_role(self) -> None:
        """Build out whatever this node's role actually implies.

        The services declared in identity.yaml have to be corroborated
        everywhere they would be on a real host: a process, a socket, a config
        under /etc, content under /srv or /var/www, and a log that is growing.
        Declaring nginx and then having no /etc/nginx is worse than not
        claiming nginx at all.
        """
        kind = self.identity["host"].get("kind", "jump")
        if kind == "web":
            self._render_web()
        elif kind == "db":
            self._render_db()
    ...
    def _render_web(self) -> None:
        self.write(
            "/etc/nginx/sites-available/erp-portal",
            "server {\n"
            "    listen 80;\n"
            "    listen [::]:80;\n"
            f"    server_name {self.fqdn} erp-web;\n"
            "\n"
            ...
            "    }\n"
            "\n"
            "    # Admin console is internal only -- do not expose in prod.\n"
            "    location /admin/ {\n"
            "        allow 10.0.0.0/8;\n"
            "        allow 172.16.0.0/12;\n"
            "        deny all;\n"
        ...
        self.path("/etc/nginx/sites-enabled").mkdir(parents=True, exist_ok=True)
        link = self.path("/etc/nginx/sites-enabled/erp-portal")
        if not link.exists():
            try:
                link.symlink_to("/etc/nginx/sites-available/erp-portal")
            except OSError:
                pass
        default = self.path("/etc/nginx/sites-enabled/default")
        if default.exists():
            default.unlink()

        self.write(
            "/var/www/erp/index.html",
            "<!doctype html>\n<html><head><title>Student ERP - Staging</title></head>\n"
            "<body>\n<h1>Student ERP Portal</h1>\n"
            "<p class=\"env\">STAGING ENVIRONMENT - not for live data</p>\n"
            "<ul>\n"
            "  <li><a href=\"/api/attendance\">Attendance</a></li>\n"
            "  <li><a href=\"/api/results\">Results</a></li>\n"
            "  <li><a href=\"/admin/\">Administration</a></li>\n"
            "</ul>\n"
            "<footer>Department of Computer Science</footer>\n"
            "</body></html>\n",
        )

        # The application nginx proxies to. Real, and it really listens.
        self.write(
            "/opt/erp/portal.py",
            "#!/usr/bin/env python3\n"
            '"""ERP staging API. Bound to localhost; nginx fronts it."""\n'
            "import json\n"
            "from http.server import BaseHTTPRequestHandler, HTTPServer\n"
            "\n"
            "DB = {'host': 'db-01', 'user': 'erp_admin', 'schema': 'erp'}\n"
            "\n"
            "\n"
            "class Handler(BaseHTTPRequestHandler):\n"
            "    server_version = 'gunicorn/21.2.0'\n"
            "\n"
            "    def log_message(self, fmt, *args):\n"
            "        with open('/var/log/erp-portal.log', 'a') as fh:\n"
            "            fh.write('%s - %s\\n' % (self.address_string(), fmt % args))\n"
            "\n"
            "    def do_GET(self):\n"
            "        if self.path.startswith('/admin'):\n"
            "            body = {'error': 'authentication required', 'realm': 'erp-admin'}\n"
            "            code = 401\n"
            "        elif self.path.startswith('/api/'):\n"
            "            body = {'status': 'ok', 'backend': DB['host'], 'schema': DB['schema']}\n"
            "            code = 200\n"
            "        else:\n"
        ...
        # The credentials that carry the chain onward to db-01. This is the
        # reward for pivoting here, and the hint for pivoting again.
        self.write(
            "/opt/erp/settings.py",
            "# ERP staging settings. Sourced by the portal service.\n"
            "DEBUG = True\n"
            "ALLOWED_HOSTS = ['erp-web', 'erp-web.cs.internal']\n"
            "\n"
            "DATABASES = {\n"
            "    'default': {\n"
            ...
            "}\n"
            "\n"
            "# TODO: rotate before go-live, ITS-2291\n"
            "SECRET_KEY = 'django-insecure-8QxTv2NmKdRw7ZpLcYh4jFgB6sVnE1aT'\n",
            mode=0o640,
        )
        # Group-readable by `deploy`. Left as root:root at 0640 the chain
        # dead-ended right here: an attacker who had done everything correctly
        # -- found the breadcrumb, escalated, stolen the key, pivoted -- hit
        # "Permission denied" on the single file the next hop depends on. A
        # deploy account that cannot read the config it deploys is also not a
        # real deploy account.
        self.run("chown", "erpadmin:deploy", str(self.path("/opt/erp/settings.py")))

        self._render_nginx_logs()

    def _render_nginx_logs(self) -> None:
        """Access logs with the shape of student traffic, not a flat rate."""
        paths = [
            "/", "/api/attendance", "/api/results", "/static/css/portal.css",
            "/static/js/app.js", "/api/attendance/CS340", "/favicon.ico",
        ]
        agents = [
        ...
        self.write(
            ...
            "\n".join(
                f"{(self.now - timedelta(days=d)).strftime('%Y/%m/%d %H:%M:%S')} [error] "
                f"{self.rng.randint(700, 999)}#0: *{self.rng.randint(1000, 9999)} "
                f"upstream timed out (110: Connection timed out) while reading response "
                f"header from upstream, server: {self.fqdn}"
                for d in range(1, 6)
            )
            + "\n",
            mode=0o640,
    ...
    def _render_db(self) -> None:
        """Seed the records an attacker came all this way for."""
        rows = []
        first = ["A", "M", "S", "K", "R", "P", "N", "V", "T", "D"]
        last = ["Bhat", "Rao", "Nair", "Iyer", "Menon", "Shetty", "Kulkarni",
                "Reddy", "Pillai", "Desai", "Hegde", "Prabhu"]
        for index in range(1, 241):
            roll = f"PES1UG22CS{index:03d}"
            name = f"{self.rng.choice(first)} {self.rng.choice(last)}"
            cgpa = round(self.rng.uniform(5.4, 9.8), 2)
            rows.append(
                f"  ('{roll}','{name}','BTech CSE',{cgpa},"
                f"'{roll.lower()}@pesu.example','9{self.rng.randint(100000000, 999999999)}')"
            )
        ...
        self.write(
            ...
            "  email VARCHAR(120)\n"
            ") ENGINE=InnoDB;\n\n"
            "INSERT IGNORE INTO students (roll,name,programme,cgpa,email,phone) VALUES\n"
            + ",\n".join(rows)
            + ";\n\n"
            "INSERT IGNORE INTO admin_users (username,password_hash,email) VALUES\n"
            "  ('erp_admin','$2y$10$K7vN2xQpZmB8wKdEyNrFhO','admin@pesu.example'),\n"
            "  ('registrar','$2y$10$T9aXqZmB4wKdEyNrFhQ2uC','registrar@pesu.example');\n\n"
            "CREATE USER IF NOT EXISTS 'erp_admin'@'%' IDENTIFIED BY 'Tr3llis!84m';\n"
            "GRANT ALL PRIVILEGES ON erp.* TO 'erp_admin'@'%';\n"
            "CREATE USER IF NOT EXISTS 'erp_readonly'@'%' IDENTIFIED BY 'R3adOnly!2026';\n"
            "GRANT SELECT ON erp.* TO 'erp_readonly'@'%';\n"
            "FLUSH PRIVILEGES;\n",
            mode=0o640,
        )

        # mysqld must accept connections from erp-web, which is the whole
        # point of the pivot. Binding to localhost would make the credentials
        # the attacker just stole useless.
        self.write(
            "/etc/mysql/mariadb.conf.d/60-erp.cnf",
            "[mysqld]\n"
            ...
            "\n"
            "# Replication account used by the nightly ERP sync\n"
            "server-id = 1\n",
        )
